...
let EasyChatController = class EasyChatController {
    constructor(websocket, myId, myNames, myPhotoUrl) {
    ...
    send(chatMessage) {
        ...
        this.messages = [...this.messages, msg];
        this.scrollToLast();
        return this.websocket.sendRequest(chat_enum_1.ECHATMETHOD.CHAT_MESSAGE, {
            id,
            roomId: this.activeRoom.id,
            chatMessage,
            createTime,
    ...
    updateStatus(status, msg) {
        return this.websocket.sendRequest(chat_enum_1.ECHATMETHOD.UPDATE_STATUS, {
            id: msg.id,
            status,
            statusField: status,
            from: this.myId,
    ...
    deleteRestoreMesage(id, deleted) {
        return this.websocket.sendRequest(chat_enum_1.ECHATMETHOD.DELETE_MESSAGE, {
            deleted,
            id,
            from: this.myId,
            to: this.toPeer
