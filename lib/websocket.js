...
const initEasyChat = (url, userId, userNames, userPhotoUrl) => {
    ...
    const easyChatController = new chat_controller_1.EasyChatController(easyChatClient, new eventbus_controller_1.EventbusController(), new logger_controller_1.LoggerController(), userId, userNames, userPhotoUrl);
...
class EasyChatClient {
    ...
    setupEventHandler(socket) {
        ...
        socket.on('disconnect', (reason) => {
            this.logger.error('WebsocketService:setupEventHandler:: - Socket disconnect, reason: %s', reason);
            this.eventbus.chat$.next({
    ...
    setupNotificationHandler() {
        ...
        socket.on('mainnotification', (request) => {
            this.logger.debug('WebsocketService:setupNotificationHandler:: - mainnotification event, method: %s,data: %o', request.method, request.data);
